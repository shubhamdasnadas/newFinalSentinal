...
const DEFAULT_BOXES: BoxLayout[] = [
  ...
  { i: "fw-explorer",   x: 0,  y: 33,  w: 7, h: 44 },
...
export default function DashboardPage() {
  ...
  const staticItems = boxes.map(b => ({ i: b.i, x: b.x, y: b.y, w: b.w, h: b.h, minW: 2, minH: 20 }));
  const widgetItems = fwWidgets.map(w => ({ i: w.id, x: Number(w.x??0), y: Number(w.y??getBottomY()+1), w: Number(w.w??6), h: Number(w.h??33), minW: 3, minH: 20 }));
  const allItems = [...staticItems, ...widgetItems];

  // Stack items vertically for small breakpoints
  ...
  const handleAddWidget = async () => {
    ...
    const res = await fetch("/api/firewall/widgets", {
      ...
      body: JSON.stringify({ reportName: fwReport, xAxis: fwXAxis, yAxis: fwYAxis, chartType: fwYAxis.length > 1 ? "mixed" : fwChartType }),
    });
    const d = await res.json();
    if (res.ok && d.widget) setFwWidgets(prev => [...prev, d.widget]);
  };
    ...
    <div className="p-3 sm:p-5 lg:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <button onClick={handleS1Sync} disabled={s1Syncing} className="inline-flex items-center gap-2 bg-emerald-600 text-white px-3 sm:px-4 py-2 rounded-xl text-sm font-medium hover:bg-emerald-700 disabled:opacity-50 transition-colors whitespace-nowrap">
            {s1Syncing ? <><div className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full" /><span className="hidden sm:inline">Syncing S1…</span></> : <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg><span className="hidden sm:inline">Sync SentinelOne</span><span className="sm:hidden">Sync S1</span></>}
          </button>
          <button onClick={handleCollect} disabled={collecting} className="inline-flex items-center gap-2 bg-indigo-600 text-white px-3 sm:px-4 py-2 rounded-xl text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 transition-colors whitespace-nowrap">
            {collecting ? <><div className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full" /><span className="hidden sm:inline">Collecting…</span></> : <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg><span className="hidden sm:inline">Collect Firewall Data</span><span className="sm:hidden">Collect</span></>}
          </button>
        </div>
      </div>
      {collectMsg && (
        <div className={`mb-4 px-4 py-3 rounded-xl text-sm border ${collectMsg.ok ? "bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 text-green-700 dark:text-green-300" : "bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800 text-red-700 dark:text-red-300"}`}>{collectMsg.text}</div>
      )}
      {s1SyncMsg && (
        <div className={`mb-4 px-4 py-3 rounded-xl text-sm border ${s1SyncMsg.ok ? "bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 text-green-700 dark:text-green-300" : "bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800 text-red-700 dark:text-red-300"}`}>{s1SyncMsg.text}</div>
      )}

      {/* ── Checkpoint Harmony Cards ─────────────────────────────────────────── */}
      <div className="mb-5">
      ...
      <div ref={containerRef} className="w-full min-w-0">
        <ResponsiveGridLayout className="layout" layouts={gridLayouts}
