...
export default function NotificationsPage() {
  ...
  useEffect(() => { fetchNotifications(); }, [activeOrgSlug]);
        ...
        <div className="space-y-3">
          {filtered.map((n) => {
            ...
            return (
              <div
                key={n._id}
                className={`border rounded-2xl p-5 flex gap-4 transition-opacity ${cfg.bg} ${cfg.border} ${n.isRead ? "opacity-60" : ""}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize flex-shrink-0 ${cfg.badge} ${cfg.badgeText}`}>
                      {n.type}
                    </span>
                  </div>
