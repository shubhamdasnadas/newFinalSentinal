...
export default function NewsPage() {
  ...
  const [input, setInput] = useState('cybersecurity')
  const [total, setTotal] = useState(0)

  useEffect(() => {
    setLoading(true)
    setError('')
    fetch(`http://localhost:8000/news?q=${encodeURIComponent(query)}&page_size=20`)
      .then(async (r) => {
    ...
    <div className="p-5 lg:p-7 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          {!loading && !error
            ...
            //   {total.toLocaleString()} results &mdash; last 7 days
            // </p>
          }
        </div>
        ...
        <form onSubmit={handleSearch} className="flex gap-2">
          <input
            ...
            className="px-3 py-2 rounded-xl text-sm border border-[var(--card-border)] bg-[var(--card-bg)] text-[var(--foreground)] placeholder-[var(--muted)] focus:outline-none focus:ring-2 focus:ring-indigo-500 w-52"
          />
          <button
            type="submit"
      ...
      {error && (
        <div className="mb-6 px-4 py-3 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-sm text-red-600 dark:text-red-400">
          ...
          <span className="block text-xs mt-0.5 text-red-400">
            Make sure the FastAPI server is running: <code>uvicorn main:app --reload</code> in the <code>app/api/news/</code> directory.
          </span>
        </div>
      )}
      ...
      {!loading && !error && articles.length === 0 && (
        <div className="flex items-center justify-center h-48">
          <p className="text-sm text-[var(--muted)]">No articles found for &ldquo;{query}&rdquo;</p>
        </div>
      )}
    </div>
  )
