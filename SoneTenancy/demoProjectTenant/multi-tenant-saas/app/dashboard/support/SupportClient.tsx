...
export default function SupportClient({ initialTickets, orgSlug, orgName }: Props) {
  const [showModal, setShowModal] = useState(false);
  const [saving, setSaving] = useState(false);
  const [filter, setFilter] = useState("all");
  const [form, setForm] = useState({ subject: "", description: "", priority: "medium" });

  const { data, mutate } = useSWR("/api/support", fetcher, {
    ...
    <div className="p-6 lg:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[var(--muted)] text-sm mt-1">{orgName} — {tickets.length} tickets</p>
        </div>
      ...
      <div className="flex gap-1 mb-5 bg-[var(--muted-bg)] p-1 rounded-xl w-fit">
        {["all", "open", "in_progress", "resolved", "closed"].map((f) => (
          <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors capitalize ${filter === f ? "bg-[var(--card-bg)] text-[var(--foreground)] shadow-sm" : "text-[var(--muted)] hover:text-[var(--foreground)]"}`}>
            {f.replace("_", " ")}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((t) => {
            ...
            return (
              <div key={t.id} className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl p-5 hover:shadow-sm transition-shadow">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-3 flex-wrap">
                      ...
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${pc.bg} ${pc.text}`}>{t.priority}</span>
                      {t.created_by && (
                      ...
                      <span className="text-xs text-[var(--muted)]">{new Date(t.created_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</span>
                    </div>
                  </div>
                </div>
              </div>
