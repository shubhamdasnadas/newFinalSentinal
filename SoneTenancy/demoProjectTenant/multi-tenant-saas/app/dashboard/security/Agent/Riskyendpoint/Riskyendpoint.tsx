...
const Riskyendpoint = ({ data }: { data: RiskyEndpointItem[] }) => {
    console.log("Riskyendpoint data:", data);
    ...
    return (
        <div className="w-full space-y-5 rounded-2xl border border-slate-200 bg-white p-4 text-slate-900 shadow-sm dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100">
            <div>
                <h2 className="text-lg font-bold">Risky Endpoint Dashboard</h2>
                ...
                <BigCard title="Risk Reason Breakdown">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                        {analytics.reasonBreakdown.map((item) => (
                            <div
                                key={item.name}
                                className="flex items-center justify-between rounded-lg bg-slate-100 px-2 py-1 dark:bg-slate-900"
                            >
                                <span>{item.name}</span>
                                <b>{item.value}</b>
                            </div>
                ...
                <BigCard title="Site Wise Risk">
                    <ResponsiveContainer width="100%" height={220}>
                        <BarChart data={analytics.siteWiseRisk}>
                            <XAxis dataKey="site" fontSize={11} />
                            <YAxis allowDecimals={false} fontSize={12} />
                            <Tooltip />
                            <Bar dataKey="inactive" stackId="a" fill={COLORS.red} />
                            <Bar
                                dataKey="firewallDisabled"
                                stackId="a"
                                fill={COLORS.orange}
                            />
                ...
                {/* <BigCard title="Endpoint Health Score">
                    <div className="flex h-[220px] flex-col items-center justify-center">
                        <div
                            ...
                            style={{
                                background: `conic-gradient(${analytics.healthScore >= 80
                                    ? COLORS.green
                                    : analytics.healthScore >= 50
                                        ? COLORS.orange
                                        : COLORS.red
                                    } ${analytics.healthScore * 3.6}deg, #e5e7eb 0deg)`,
                            }}
                        >
                    ...
                    <div className="space-y-3">
                        {analytics.leaderboard.map((item, index) => (
                            <div
                                ...
                                className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 dark:border-slate-800 dark:bg-slate-900"
                            >
                                <div>
                                ...
                                <span className="rounded-full bg-red-100 px-3 py-1 text-sm font-bold text-red-600 dark:bg-red-950 dark:text-red-300">
                                    {item.riskScore || 0}
                                </span>
                            </div>
                        ))}
...
const SummaryCard = ({
    ...
    const colorMap = {
        ...
        green:
            "border-green-200 bg-green-50 text-green-600 dark:border-green-900 dark:bg-green-950/40 dark:text-green-300",
        blue: "border-blue-200 bg-blue-50 text-blue-600 dark:border-blue-900 dark:bg-blue-950/40 dark:text-blue-300",
        orange:
            "border-orange-200 bg-orange-50 text-orange-600 dark:border-orange-900 dark:bg-orange-950/40 dark:text-orange-300",
    };

    return (
        <div
            className={`rounded-2xl border p-4 shadow-sm ${colorMap[color]}`}
        >
