...
export default function ProjectsPage() {
  ...
  useEffect(() => { fetchProjects(); }, [activeOrgSlug]);
    ...
    return (
      <div className="p-8">
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-6">
          ...
          <p className="text-amber-700 dark:text-amber-400 mt-1 text-sm">Select an organization to view projects.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 lg:p-8">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-[var(--muted)] text-sm mt-1">
            {activeOrgName} — {activeCount} active of {projects.length} total
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="flex bg-[var(--muted-bg)] rounded-xl p-1 border border-[var(--card-border)]">
            <button
              ...
              className={`p-1.5 rounded-lg transition-colors ${view === "grid" ? "bg-[var(--card-bg)] text-[var(--foreground)] shadow-sm" : "text-[var(--muted)]"}`}
            ...
            <button
              ...
              className={`p-1.5 rounded-lg transition-colors ${view === "list" ? "bg-[var(--card-bg)] text-[var(--foreground)] shadow-sm" : "text-[var(--muted)]"}`}
      ...
      {loading ? (
        ...
        <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl p-16 text-center">
          ...
          <p className="text-[var(--muted)] text-sm mb-6">Create your first project for {activeOrgName}.</p>
          {canManage && (
            <button
              onClick={() => setShowModal(true)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {projects.map((p) => {
            ...
            return (
              <div key={p._id} className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 bg-indigo-100 dark:bg-indigo-900/40 rounded-xl flex items-center justify-center text-sm font-bold text-indigo-600 dark:text-indigo-400 flex-shrink-0">
                      {p.key?.slice(0, 2)}
                    </div>
                    <div>
                ...
                <div className="flex items-center justify-between text-xs text-[var(--muted)] pt-3 border-t border-[var(--card-border)]">
                  <span className="truncate max-w-[120px]">{p.createdBy || "—"}</span>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-[var(--card-border)]">
              {projects.map((p) => {
                const sc = STATUS_CONFIG[p.status] || STATUS_CONFIG.inactive;
                return (
                  <tr key={p._id} className="hover:bg-[var(--muted-bg)] transition-colors">
                    ...
                    <td className="px-6 py-4 hidden sm:table-cell">
                      <span className="text-xs font-mono bg-[var(--muted-bg)] text-[var(--muted)] px-2 py-1 rounded-lg">{p.key}</span>
                    </td>
                    <td className="px-6 py-4">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl shadow-2xl w-full max-w-md p-8">
            ...
            <p className="text-sm text-[var(--muted)] mb-6">Create a project in {activeOrgName}</p>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setForm((p) => ({
                      ...p,
                      name,
                      key: p.key || name.slice(0, 6).toUpperCase().replace(/\s+/g, ""),
                    }));
                  }}
                  required
