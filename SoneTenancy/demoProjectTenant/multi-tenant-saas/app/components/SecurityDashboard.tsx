...
const DEFAULT_BOXES: BoxLayout[] = [
  { i: "mitigation", x: 0,  y: 0,   w: 5, h: 33 },
  { i: "severity",   x: 5,  y: 0,   w: 4, h: 33 },
  { i: "threats",    x: 9,  y: 0,   w: 3, h: 33 },
  { i: "firewall",   x: 0,  y: 33,  w: 7, h: 44 },
  { i: "fwthreat",   x: 7,  y: 33,  w: 5, h: 44 },
  { i: "agents",     x: 0,  y: 77,  w: 5, h: 33 },
];
...
export default function SecurityDashboard() {
  ...
  useEffect(() => {
    ...
    fetch("/api/dashboard/layout", { credentials: "include" })
      .then(r => r.json())
      .then(d => {
        const saved = Array.isArray(d.layout?.secboxes) ? d.layout.secboxes : [];
        const merged = DEFAULT_BOXES.map(def => saved.find((b: BoxLayout) => b.i === def.i) ?? def);
        setState(prev => ({ ...prev, ...(d.layout?.secopts || {}), boxes: merged }));
      })
      .catch(() => {})
  ...
  const persistState = useCallback((next: PageState) => {
    ...
    saveTimerRef.current = setTimeout(() => {
      fetch("/api/dashboard/layout", {
        ...
        credentials: "include",
        body: JSON.stringify({ layout: { secboxes: next.boxes, secopts: { mitigationChart: next.mitigationChart, firewallReport: next.firewallReport } } }),
      })
  ...
  const updateState = useCallback((patch: Partial<PageState>) => {
    setState(prev => { const next = { ...prev, ...patch }; persistState(next); return next; });
  }, [persistState]);
  ...
  const getBottomY = () => Math.max(0, ...[...state.boxes, ...firewallWidgets.map(w => ({ y: Number(w.y ?? 0), h: Number(w.h ?? 5) }))].map(i => i.y + i.h));

  // ── Grid layout items — all breakpoints ────────────────────────────────────
  const staticItems = state.boxes.map(b => ({ i: b.i, x: b.x, y: b.y, w: b.w, h: b.h, minW: 3, minH: 20 }));
  const widgetItems = firewallWidgets.map(w => ({ i: w.id, x: Number(w.x ?? 0), y: Number(w.y ?? getBottomY() + 1), w: Number(w.w ?? 6), h: Number(w.h ?? 33), minW: 3, minH: 20 }));
  const allItems = [...staticItems, ...widgetItems];

  const stackVertically = (items: typeof allItems, cols: number) =>
    items.map((item, idx) => ({ ...item, x: 0, y: idx * item.h, w: cols }));

  const gridLayouts = {
    lg: allItems,
    md: allItems.map(b => ({ ...b, w: Math.min(b.w, 10) })),
    sm: stackVertically(allItems, 6),
    xs: stackVertically(allItems, 4),
    xxs: stackVertically(allItems, 2),
  };
  ...
  const handleLayoutChange = (newLayout: Layout, _allLayouts: Partial<Record<string, Layout>>) => {
    ...
    setFirewallWidgets(prev => prev.map(widget => {
    ...
    <div className="p-3 sm:p-5 lg:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-xs text-[var(--muted)] mt-0.5 hidden sm:block">Drag to reposition · Resize from corners · Add custom widgets below</p>
        </div>
      ...
      <div ref={containerRef} className="w-full min-w-0">
        ...
        >
              ...
              <div className="flex gap-1">
                {(["donut","probability","bar"] as const).map(ct => (
                  <button key={ct} onClick={e => { e.stopPropagation(); updateState({ mitigationChart: ct }); }}
                    className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${state.mitigationChart === ct ? "bg-indigo-600 text-white" : "bg-[var(--card-bg)] border border-[var(--card-border)] text-[var(--muted)] hover:bg-[var(--muted-bg)]"}`}>
          ...
          <div key="agents" className="bg-[var(--card-bg)] rounded-2xl border border-[var(--card-border)] shadow-sm flex flex-col overflow-hidden">
            <div className="drag-handle cursor-grab active:cursor-grabbing bg-[var(--muted-bg)] border-b border-[var(--card-border)] px-4 py-3 flex items-center justify-between flex-shrink-0 select-none">
              ...
              <div className="flex gap-2">
                <span className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300">{activeCount} Active</span>
                <span className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300">{inactiveCount} Inactive</span>
              </div>
            </div>
            <div className="flex-1 min-h-0 overflow-auto">
              {agentInfoLoading ? <Spin /> : agentInfoError ? <Err msg={agentInfoError} /> : agentInfoData.length === 0 ? <Empty msg="No agent info found" /> : (
                <table className="w-full text-sm border-collapse">
                  <thead className="sticky top-0 z-10 bg-[var(--muted-bg)]">
                    <tr>
                      <th className="text-left px-4 py-3 font-semibold text-[var(--muted)] border border-[var(--card-border)]">Computer</th>
                      <th className="text-left px-4 py-3 font-semibold text-[var(--muted)] border border-[var(--card-border)]">Active</th>
                    </tr>
                  </thead>
                  <tbody>
                    {agentInfoData.map((a, i) => (
                      <tr key={a.id || i} className={i % 2 === 0 ? "bg-[var(--card-bg)]" : "bg-[var(--muted-bg)]"}>
                        <td className="px-4 py-3 text-[var(--muted)] border border-[var(--card-border)] whitespace-nowrap text-xs">{a.computerName || "—"}</td>
          ...
          <div key="firewall" className="bg-[var(--card-bg)] rounded-2xl border border-[var(--card-border)] shadow-sm flex flex-col overflow-hidden">
            ...
            {/* Header — drag handle wraps the whole toolbar */}
            <div className="drag-handle cursor-grab active:cursor-grabbing flex-shrink-0 select-none">
              ...
              <div className="px-4 py-2.5 bg-[var(--muted-bg)] flex flex-wrap items-end gap-2">
                ...
                <div className="flex flex-col gap-0.5 relative">
                  ...
                  <button type="button" onClick={e => { e.stopPropagation(); setShowXDrop(p => !p); setShowYDrop(false); }}
                ...
                <div className="flex flex-col gap-0.5 relative">
                  ...
                  <button type="button" onClick={e => { e.stopPropagation(); setShowYDrop(p => !p); setShowXDrop(false); }}
                    ...
                    <svg className="w-3 h-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" /></svg>
                ...
                <div className="flex flex-col gap-0.5">
                  ...
                  <div className="flex h-8 rounded-lg border border-[var(--input-border)] overflow-hidden bg-[var(--card-bg)]">
                    {(["bar","line","mixed"] as const).map(ct => (
                      <button key={ct} type="button" onClick={e => { e.stopPropagation(); setSelectedChartType(ct); }}
                ...
                <div className="flex flex-col gap-0.5">
                  ...
                  <button onClick={e => { e.stopPropagation(); handleAddWidget(); }}
                ...
                <div className="px-4 py-2 flex flex-wrap gap-1.5 border-t border-[var(--card-border)] bg-[var(--card-bg)]">
                  {selectedXAxis.map(x => (
                    <span key={x} className="inline-flex items-center gap-1 pl-2 pr-1 py-0.5 rounded-full text-[10px] font-semibold bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 border border-blue-200 dark:border-blue-700">
                      ...
                      <button onClick={e => { e.stopPropagation(); setSelectedXAxis(p => p.filter(v => v !== x)); }} className="w-3.5 h-3.5 rounded-full hover:bg-blue-200 flex items-center justify-center ml-0.5">×</button>
                    </span>
                  ))}
                  {selectedYAxis.map(y => (
                    <span key={y} className="inline-flex items-center gap-1 pl-2 pr-1 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-700">
                      ...
                      <button onClick={e => { e.stopPropagation(); setSelectedYAxis(p => p.filter(v => v !== y)); }} className="w-3.5 h-3.5 rounded-full hover:bg-emerald-200 flex items-center justify-center ml-0.5">×</button>
                    </span>
                  ))}
                </div>
              )}
