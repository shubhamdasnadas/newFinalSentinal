/**
 * GET /api/sentinelone/risks
 * Fetches ALL application management risks with pagination + 429 handling.
 */
...
export async function GET(req: NextRequest) {
  try {
    ...
    do {
      const url = new URL(`${cleanBase}/web/api/v2.1/application-management/risks`);
      ...
      console.log(
        `[S1] Page fetched: ${pageData.length}, Total: ${allData.length}/${totalItems}`
      );

      await sleep(1000);
    } while (nextCursor);

    // ✅ ADDED: Store fetched risks into DB
    if (allData.length > 0) {
      await SentinelOneModel.upsertRisks(orgSlug, allData);
    }
